...
## Study session: TypeORM

Run 2026-09-05/06 against the real repository, on copies of
`apps/api/data/neuron.db`. Working tree clean at the end; the real database was
never opened for writing.

Scoring is the three-step sequence: **step 1** answered from an open question,
**step 2** answered after one narrowing question, **step 3** needed teaching.

| # | Topic | Step |
|---|-------|------|
| 1 | An entity is a class that registers itself | **1** |
| 2 | `select: false` and the ownership check | **1** |
| 3 | The repository boundary / `@InjectRepository` | **1** (4 of 5 predictions correct) |
| 4 | `Raw()` and why `Like()` was not enough | **1** |
| 5 | `synchronize: false` | **1** (partial on one sub-question) |
| 6 | Migrations and the baseline problem | **1** |
| 7 | What it all cost | **1** |

Seven of seven at step 1. No topic required teaching before she produced the
mechanism. Recorded honestly: the misses below are real, and none of them
changed the step score, because in each case the mechanism was correct and the
gap was in a detail she had no way to read off the code in front of her.

### What she got right, and where the gaps were

**1 — Decorators register.** Answered before any experiment: decorators run once
when the class is defined, not on instantiation or use, so the count is one per
decorated property at import time. Correct, and she volunteered the reason
rather than the rule. The compiled `entry.entity.js` confirmed it — five
top-level `__decorate(...)` calls, `@Entity` applied last, after the property
decorators. A probe showed a bare `require` of the entity populating
`getMetadataArgsStorage()` with two tables and four columns, and five `new`
instances leaving the count at four.

*Note:* the study prompt refers to `entry.interface.ts`; the file is now
`entry.entity.ts` (renamed in 95ec3db). The prompt predates the rename.

**2 — `select: false`.** She predicted the column is excluded from the SELECT
and that both ownership checks therefore deny. **The prompt's premise was
wrong and she was right.** The prompt asserts version B
(`entry.userId !== callerId → deny`) "passes for everybody and hands every
journal to every user." With `callerId` guaranteed to be a string,
`undefined !== callerId` is pinned to `true`, so B denies everyone — a
denial-of-service, not a data leak. She held this position when pushed toward
the prompt's answer twice, and the experiment confirmed her: both A and B
returned DENY.

The real danger is narrower and worth restating in the prompt: **the check
silently stops being a check.** `entry.userId` is `undefined` for structural
reasons, so any comparison against it is a constant. A Day 10 test asserting
"Bob cannot read Alice's entry" passes — because the check denies everyone, not
because it identified Bob.

Third finding, from the same experiment: the key is **present holding
`undefined`**, not absent. `Object.keys()` lists `userId`; `'userId' in loaded`
is `true`. The entity comment saying the property is "simply absent" is
inaccurate. It makes no difference to a comparison, and it is why
`JSON.stringify` still omits it from response bodies.

**3 — The boundary.** Her explanation of `@InjectRepository` was complete at step
1, including the collision that makes it necessary: `Repository<User>` and
`Repository<JournalEntry>` are the same runtime value, so the erased type
argument cannot name a token.

Prediction on the five checks: typecheck pass, **lint fail**, build pass, unit
pass, e2e pass. Actual: **all five green.** A deliberate violation — `Repository`
imported into `entries.service.ts`, injected, raw SQL written without
`escapeLikePattern`, `EntriesRepository` bypassed entirely — produced
`typecheck ✅ lint ✅ build ✅ 108 unit ✅ 35 e2e ✅`. `eslint.config.mjs` has no
`no-restricted-imports` rule; nothing in the toolchain can see the violation. The
violating code sat directly beneath the comment forbidding it.

Her closing sentence was right in substance — no compiler, no runtime, no test
catches it — and wrong only in attributing the catch to lint.

**4 — `Raw()` vs `Like()`.** Both parts at step 1, and the second was exact:
`escapeLikePattern` writes the marker (`%` → `\%`), `ESCAPE '\'` is what gives
the marker meaning. Four variants run side by side on seeded data confirmed it.
`Like()` emits `LIKE ?` with no `ESCAPE` clause, so the escaped pattern
`%\%%` makes SQLite search for a **literal backslash** — searching for `%`
returned the entry containing a backslash, and searching for `100%` returned
nothing.

Unnoticed by both of us beforehand: for the input `100%`, escaped and unescaped
agree (both return the one row), because the stray wildcard has nothing else to
match. The bug is invisible for that input, which is why the suite tests a bare
`%`. Removing `escapeLikePattern` from the real code failed exactly three unit
tests, all named in user vocabulary — "should return only entries containing a
literal percent sign" — none mentioning LIKE, ESCAPE or backslashes.

**5 — `synchronize`.** Questions 2 and 3 at step 1, and her phrasing anticipated
the codebase's own: the weak test asserts the configuration, "testing how the
developer implemented the safety, not the safety itself"; the strong test boots
against an existing schema and verifies it did not change. `test/synchronize.e2e-spec.ts`
makes the same argument in a comment — *"the claim above says what the
configuration is. This one says what it does, and it is the assertion that would
still be true if TypeORM renamed the option tomorrow."*

Question 1 was **partial**. She predicted the three additions — `users` table,
`user_id` column, foreign key — all correct. She described it as creating the
missing pieces. What it actually does is `DROP TABLE "entries"` **three times**,
rebuilding through `temporary_entries` on each pass. The data survived here
because the copy step listed the right columns; the safety depends entirely on
TypeORM's diff being right, and a renamed property reads as drop-plus-add.
Afterwards there was **no `migrations` table** — the schema changed with no
record of how.

Setting `synchronize: true` in the real module now fails **3 tests** (1 unit, 2
e2e), against the audit's pre-8b `96 unit ✅ 32 e2e ✅`. The gap is closed.

**6 — The baseline problem.** The strongest answer of the session, and it
matters because **her real database has this fault right now**. Unprompted, she
gave the whole chain: `migration:run` reads the history table rather than the
entities; finding none, it treats both migrations as pending; it runs
`InitialSchema` first; `CREATE TABLE "entries"` fails because the table is
there; the five entries survive.

Confirmed line by line in the log, including exit code 1, `ROLLBACK`, and the
error text. Her one wrong assumption was that `InitialSchema` "presumably
contains `CREATE TABLE IF NOT EXISTS`" — it does not, deliberately, and that
absence is what makes the failure loud instead of silent. One addition she did
not have: TypeORM *creates* the empty `migrations` table on the way, so a
missing history and a zero-migration history are indistinguishable to it.

The README repair (one row) was applied to the copy: exit 0, `users` created,
five entries intact with `user_id` NULL, foreign key present, `PRAGMA
foreign_keys = 1`. The `AddUsersAndEntryOwnership` migration is 124 lines and
drops `entries` twice.

**7 — The cost.** At step 1, with an addition beyond the question. The Day 7
statement was run against a copy of her real journal and a fresh file, and
reproduced ADR-010's founding demonstration exactly:

```
EXISTING (her real journal)  columns: id, content, created_at            <- skipped
FRESH   (new clone)          columns: id, content, created_at, user_id   <- ran
```

Same code, two schemas, decided by whether a file existed. Silent on both.

She named the purchase correctly — a tracked history of intentional changes that
can be applied to databases that already exist — and then went further than the
prompt asked: the capability **depends on that history being aligned with the
database**, so adopting migrations onto a database that predates them creates a
one-time historical problem. That is the right generalisation. TypeORM never
inspects the schema to decide what to run; it reads the ledger and trusts it. An
empty ledger against a populated database fails loudly (her case); a ledger
claiming migrations that never ran **skips** them, which is Day 7's silent
failure reintroduced through the record instead of the statement.

### Outstanding

- **`apps/api/data/neuron.db` is still un-baselined.** The next
  `pnpm migration:run` against it will fail as demonstrated. The repair is the
  README block under *A database created before migrations existed*, pointed at
  `data/neuron.db`. Deliberately not run during a study session.
- **The prompt's topic-2 premise should be corrected** before it is reused.
- **The entity comment's "simply absent"** is inaccurate; the property is
  present and `undefined`.
- No `migration:baseline` script exists; the repair is manual.

### For the teacher

She answered every topic at step 1 and defended a correct answer against two
rounds of pressure toward a wrong one. Where she was incomplete it was on
physical detail invisible from the source — that `synchronize` rebuilds rather
than alters, that `InitialSchema` has no `IF NOT EXISTS`, that lint has no
boundary rule. None of those are reasoning failures; they are facts that have to
be read or run. The habit of working from mechanism rather than recall is
present and was not present three days ago.
